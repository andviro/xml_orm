...
Все поля задаются с помощью классов-потомков `core.SimpleField` и
`core.ComplexField`. Для простых полей (наследующих от `SimpleField`)
необязательный параметр `tag` задает название соответствующего элемента или (с
префиксом `@`) -- атрибута. Если параметр `tag` не указан, поле считывается и
записывается из текстового содержимого узла. Корректно отрабатываются несколько
...
### Пoля `core.FloatField`

Тип, соответствующий типу `float`.

### Пoля `core.IntegerField`

Тип, соответствующий типу `int`.

### Пoля `core.DecimalField`

Тип, соответствующий типу `decimal.Decimal`. Имеет параметры конструктора
`max_digits` и `decimal_places`.

### Пoля `core.ComplexField`

Хранят сложные элементы, имеющие собственную схему. Класс для схемы передается
в первом, обязательном параметре конструктора. Класс схемы сложного
поля имеет свое пространство имен. Если параметр конструктора `use_schema_ns`
равен `True`, идентификатор пространства имен перекрывается идентификатором,
заданным в схеме, в которую добавляется поле. По умолчанию этот параметр равен
`False`, т.е. пространство имен сложного элемента определяется его собственным
классом схемы.

### Пoля `core.RawField`

Соответствуют типу содержимого `xs:any`. Значением поля является объект
`etree.Element`.
